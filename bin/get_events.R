...
veg_ids_start <- dta[month == 6, ID]
veg_ids_end <- dta[month == 8, ID]
veg_dr <- dta[ID %in% veg_ids_start & ID %in% veg_ids_end & dur == 3]
...
veg_dr <- dta[ID %in% veg_ids_start & ID %in% veg_ids_end & dur == 5]
...
veg_dr <- dta[ID %in% veg_ids_start & ID %in% veg_ids_end]
...
veg_ids_start <- dta[month == 3, ID]
veg_ids_end <- dta[month == 9, ID]
veg_dr <- dta[ID %in% veg_ids_start & ID %in% veg_ids_end & dur == 7]
